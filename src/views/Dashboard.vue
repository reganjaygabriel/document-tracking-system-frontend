        ...
        <div class="p-6 space-y-6">
          ...
          <div>
            ...
            <div class="grid grid-cols-3 gap-3">
              ...
              <label 
                class="flex flex-col items-center p-4 border-2 rounded-xl cursor-pointer transition-all duration-200 hover:border-primary-400 hover:bg-primary-50"
                :class="reportFormat === 'excel' ? 'border-primary-500 bg-primary-50' : 'border-gray-200'"
              >
                <input 
                  type="radio" 
                  v-model="reportFormat" 
                ...
                />
                <span class="text-3xl mb-2">📊</span>
                <span class="font-semibold text-gray-900">Excel</span>
              ...
              >
                ...
                <span class="text-xs text-gray-500 mt-1">Data File</span>
              </label>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    generateExcelReport() {
      ...
      csvContent += `Total Documents,${this.stats[0].value}\n`;
      csvContent += `Pending Review,${this.stats[1].value}\n`;
      csvContent += `Approved,${this.stats[2].value}\n`;
      csvContent += `Rejected,${this.stats[3].value}\n\n`;
      
      // Add document list
      csvContent += `Document List\n`;
      csvContent += `Name,Type,Size,Owner,Status,Category,Last Modified\n`;
      
      this.documents.forEach(doc => {
        csvContent += `"${doc.name}","${doc.file_type}","${this.formatFileSize(doc.file_size)}","${doc.owner_name}","${doc.status}","${doc.category || 'Uncategorized'}","${this.formatDate(doc.updated_at)}"\n`;
      });
